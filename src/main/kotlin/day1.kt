...
fun part1(input: String): Int {
    ...
    for (line in lines) {
        val noLetters = line.filter { c -> c.isDigit() }
        val first = noLetters.first()
        val last = noLetters.last()
        val numeric = "$first$last".toInt()
        total += numeric
    }
...
fun part2(input: String): Int {
    ...
    for (line in lines) {
        val lineMut = line.replace(Regex("(one|two|three|four|five|six|seven|eight|nine)")) {
            numberMap[it.value]!!.toString()
        }

        val noLetters = lineMut.filter { c -> c.isDigit() }
        val first = noLetters.first()
        val last = noLetters.last()
        val numeric = "$first$last".toInt()
        total += numeric
...
fun main() {
    println("Part 1: ${part1(PUZZLE_IN)}")
    println("Part 2: ${part2(PUZZLE_IN)}")
}